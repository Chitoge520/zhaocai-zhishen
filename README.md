...
已完成 M6 竞赛驾驶舱升级：支持项目风险穿透、网络/报价/跨项目共现三类风险贡献、项目与全局关系图、HTML/DOCX 风险融合摘要，以及断网可用的离线比赛快照。

启动服务后，访问首页并点击“加载离线演示”，系统会通过 `/api/demo` 加载固定的、完全虚构、完全脱敏、预计算的快照。它不读取真实项目材料，不触发 OCR、训练、大模型或网络调用，并可完整展示全样本概览、三类信号、风险融合、关系图谱和双栏证据回放。

该快照仅用于展示产品流程和可解释性，不得用其宣称真实案件识别效果、准确率，也不得将其中的虚构线索认定为围标串标结论。
