...
- `quote-analysis-example.jsonl`：用于 M3 端到端验收的报价聚类示例，演示总价/分项报价隔离、单位归一、规律性差异、缺失控制价和未知单位排除。
...
| 字段 | 用途 |
| --- | --- |
| amount / total_price | 总价；仅与同项目、同币种的其他总价比较 |
| control_amount / control_price | 控制价或预算上限；缺失时不阻断其他比较 |
| amount_unit | 金额单位，如 元、千元、万元、CNY；未知单位会被排除 |
| quote_scope | total 或 item，明确区分总价与分项报价 |
| item_code / item_name | 分项对齐键 |
| quantity / unit_price | 分项数量与单价；分项相关性只使用共同分项 |
...
.\.venv\Scripts\python.exe -m zhaocai_zhishen analyze-quotes `
  --input data/audit_ingestion_template_check `
  --output data/quote_analysis_template_check
```
...
- M3 中的单一报价接近、尾数、固定差额、统计偏离或分项相关性均不能直接定性；总价与分项报价不会混用；
